# Code from vioplot package, modified to take a list as input and some changes to coloring.
# Written by Åsa Björklund asa.bjorklund@scilifelab.se 2016

vioplot.list<- function (datas, range = 1.5, h = NULL, ylim = NULL, names = NULL,
    horizontal = FALSE, col = "magenta", border = "black", lty = 1,
    lwd = 1, rectCol = "black", colMed = "white", pchMed = 19,
    at, add = FALSE, wex = 1, drawRect = TRUE,las=2)
{
    n <- length(datas)
    if (missing(at))
        at <- 1:n
...
# make one function with no border and no mean/quantile plotting
# also remove axes.
    ...
    at, add = FALSE, wex = 1, drawRect = TRUE, las=2, ylab=NULL)
{
    ...
    if (!horizontal) {
	    ...
	    #axis(1)
            axis(1, at = at, labels = label,tick=FALSE,col="white", las=las)
        }
        box(col="white")
	if(length(col)!=n){ col<-rep(col[1],n) }
        for (i in 1:n) {
